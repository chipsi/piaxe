...
void SERIAL_send(uint8_t *data, int len, bool debug) {
    for (int i=0;i<len;i++) {
        printf("%02x", data[i]);
    }
    printf("\n");
...
int SERIAL_rx(uint8_t *data, int len, int timeout) {
    return 0;
}
...
void BM1366_send_hash_frequency(float target_freq)
{
    ...
    printf("Setting Frequency to %.2fMHz (%.2f)\n", target_freq, newf);
}
...
int BM1366_set_max_baud(void)
{
    ...
    printf("Setting max baud of 1000000 \n");
...
void BM1366_set_job_difficulty_mask(int difficulty)
{
    ...
    printf("Setting job ASIC mask to %d\n", difficulty);
...
task_result * BM1366_proccess_work(void * pvParameters)
{
    ...
    uint32_t rolled_version = 0; // TODO GLOBAL_STATE->ASIC_TASK_MODULE.active_jobs[rx_job_id]->version;
