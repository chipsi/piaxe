...
def hex2bin(hex_str, bin_len):
    bin_array = bytearray(bin_len)
    len_hex = len(hex_str)
    idx_bin = 0

    for i in range(0, len_hex, 2):
        if i + 1 < len_hex:
            bin_array[idx_bin] = hex2val(hex_str[i]) << 4 | hex2val(hex_str[i+1])
        else:
            # In case of an odd number of hex characters
            bin_array[idx_bin] = hex2val(hex_str[i]) << 4
        idx_bin += 1
        if idx_bin >= bin_len:
            break

    return bin_array[:idx_bin]
